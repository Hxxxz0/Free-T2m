...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.lambda_award = lambda_award
        ...
        self.eval_wrapper = EvaluatorMDMWrapper('humanml',dist_util.dev() )
    ...
    def remove_lf(self,batch):
        ...
        return torch.cat(vecs_list, dim=-1).reshape(bs,-1,251)
    
    # def remove_lf(self, batch, dim=5, draw=False):
    #     vecs_list = []  

    #     for sample in batch.split(1, dim=0):
    #         sample_vecs = [] 
            
    #         for vec in sample.split(1, dim=-1):

    #             vec_f = dct.dct(vec.reshape(-1,1)) 
    #             vec_f[dim:] = 0  
    #             vec_lf = dct.idct(vec_f)  
    #             sample_vecs.append(vec_lf)  

    #         vecs_list.append(torch.cat(sample_vecs, dim=-1).unsqueeze(0))


    #     return torch.cat(vecs_list, dim=0)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            mlens = []
            for i in model_output.squeeze(2).permute(0,2,1):
                n =0 
                for j in i:
                    if j.sum() !=0:
                        n+=1
                mlens.append(n)
            motion_embedding = self.eval_wrapper.get_motion_embeddings_train(model_output.squeeze(2).permute(0,2,1), m_lens=torch.tensor(mlens))
            mlens = []
            for i in x_start.squeeze(2).permute(0,2,1):
                n =0 
                for j in i:
                    if j.sum() !=0:
                        n+=1
                mlens.append(n)
            target_motion_embedding = self.eval_wrapper.get_motion_embeddings_train(x_start.squeeze(2).permute(0,2,1), m_lens=torch.tensor(mlens))
            ...
            l_mask = torch.tensor(l_mask).to(x_start.device)
            s_mask = torch.tensor(s_mask).to(x_start.device)
            
            target_xyz, model_output_xyz = None, None
            if self.lambda_semantic>0:
                cosine_similarity = F.cosine_similarity(target_motion_embedding.to(x_start.device), motion_embedding.to(target_motion_embedding.to(x_start.device)), dim=1)
                terms["semantic_award"] = 1 - cosine_similarity
                terms["semantic_award"] = terms["semantic_award"] * s_mask
                

            if self.lambda_award > 0:
                bs = target.shape[0]
                # target_re =
                gt_lf = self.remove_lf(target.squeeze().permute(0, 2, 1).to(motion_embedding.device))
                model_output__lf = self.remove_lf(model_output.squeeze().permute(0, 2, 1).to(motion_embedding.device))
                terms['award'] = self.lf_loss(gt_lf,model_output__lf)
                terms['award'] = terms['award'] * l_mask
            ...
            terms["loss"] = terms["rot_mse"] + terms.get('vb', 0.) +\
                            ...
                            (self.lambda_award * terms.get('award', 0.)) + \
                            (self.lambda_semantic * terms.get('semantic_award', 0.)) 
