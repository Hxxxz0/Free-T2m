...
def build_models(opt):
    ...
    movement_enc = MovementConvEncoder(263-4, 512, 512)
    text_enc = TextEncoderBiGRUCo(word_size=300,
                                  pos_size=15,
                                  hidden_size=512,
                                  output_size=opt.dim_coemb_hidden,
                                  device=opt.device)
    ...
    motion_enc = MotionEncoderBiGRUCo(input_size=opt.dim_movement_latent,
                                      ...
                                      output_size=opt.dim_coemb_hidden,
                                      device=opt.device)
    checkpoint = torch.load("/home/user/dxc/motion/StableMoFusion/data/pretrained_models/t2m/text_mot_match/model/finest.tar",
                            map_location=opt.device)
    movement_enc.load_state_dict(checkpoint['movement_encoder'])
    text_enc.load_state_dict(checkpoint['text_encoder'])
    motion_enc.load_state_dict(checkpoint['motion_encoder'])
...
class EvaluatorModelWrapper(object):
    ...
    def get_motion_embeddings_train(self, motions, m_lens):
            with torch.no_grad():
                motions = motions.detach().float()

                align_idx = np.argsort(m_lens.data.tolist())[::-1].copy()
                align_idx_revert = np.argsort(align_idx)
                motions = motions[align_idx]
                m_lens = m_lens[align_idx]
                ...
                m_lens = m_lens // 4
                motion_embedding = self.motion_encoder(movements, m_lens)
                motion_embedding = motion_embedding[align_idx_revert]
                return motion_embedding